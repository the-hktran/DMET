...
Eigen::VectorXd CalcGradL(InputObj &Input, std::vector< Eigen::MatrixXd > FragmentDensities, std::vector< Eigen::MatrixXd > InitialDensities, std::vector< std::vector< double > > &PotentialElements, std::vector< std::vector< std::pair<int, int> > > &PotentialPositions, std::vector< Eigen::MatrixXd > &FragmentRotations, std::vector< int > BathStates, std::vector< std::vector< int > > OccupiedByState, std::vector< std::vector< int > > VirtualByState)
{
    ...
    for (int x = 0; x < Input.NumFragments; x++)
	{
		for (int i = 0; i < PotentialElements[x].size(); i++)
		{
            ...
            for (int ii = 0; ii < Input.NumSoln; ii++)
            {
                double SCFEnergy = SCF(Bias, ii + 1, DensityMatrix, Input, BlankOutput, Input.SOrtho, Input.HCore, AllEnergies, CoeffMatrix, OccupiedOrbitals, VirtualOrbitals, SCFCount, Input.MaxSCF, DMETPotPlusDU, OrbitalEV);
                ...
                SCFEnergy *= -1;
                SCFMDEnergyQueue.push(std::pair<double, int>(SCFEnergy, ii));
...
double doLineSearch(InputObj &Input, std::vector< Eigen::MatrixXd > &FragmentDensities, std::vector< Eigen::MatrixXd > &FullDensities, std::vector< std::vector< double > > PotentialElements, std::vector< std::vector < std::pair< int, int > > > PotentialPositions, Eigen::VectorXd p, Eigen::MatrixXd DMETPotential, std::vector< Eigen::MatrixXd > &FragmentRotations, std::vector< int > BathStates, std::vector< std::vector< int > > OccupiedByState, std::vector< std::vector< int > > VirtualByState)
{
    ...
    for (int ii = 0; ii < Input.NumSoln; ii++)
    {
        double SCFEnergy = SCF(Bias, ii + 1, DensityMatrix, Input, BlankOutput, Input.SOrtho, Input.HCore, AllEnergies, CoeffMatrix, OccupiedOrbitals, VirtualOrbitals, SCFCount, Input.MaxSCF, IncrementedDMETPot, OrbitalEV);
        std::tuple< Eigen::MatrixXd, double, double > tmpTuple = std::make_tuple(DensityMatrix, Input.StartNorm, Input.StartLambda); // Add a new bias for the new solution. Starting N_x and lambda_x are here.
        Bias.push_back(tmpTuple);
        SCFEnergy *= -1;
        SCFMDEnergyQueue.push(std::pair<double, int>(SCFEnergy, ii));
    ...
    {
        int NextIndex = SCFMDEnergyQueue.top().second;
        DNext[a] = 2 * SCFMD1RDM[NextIndex];
        SCFMDEnergyQueue.pop();
        // **************** Orbital lock method below **********************
    ...
    do // while we're decreasing L along the step direction
    {
        ...
        // std::vector< double > AllEnergies1;
        // std::vector< std::tuple < Eigen::MatrixXd, double, double > > Bias1;
        std::vector< Eigen::MatrixXd > SCFMD1RDM1;
        std::priority_queue< std::pair< double, int > > SCFMDEnergyQueue1;
        std::vector< std::vector< int > > SCFMDOccupied1;
        std::vector< std::vector< int > > SCFMDVirtual1;
        std::vector< Eigen::MatrixXd > SCFMDCoeff1;
        std::vector< Eigen::VectorXd > SCFMDOrbitalEV1;

        // First, run SCF some number of times to find a few solutions.
        for (int ii = 0; ii < Input.NumSoln; ii++)
        {
            double SCFEnergy = SCF(Bias1, ii + 1, DensityMatrix, Input, BlankOutput, Input.SOrtho, Input.HCore, AllEnergies1, CoeffMatrix, OccupiedOrbitals1, VirtualOrbitals1, SCFCount, Input.MaxSCF, IncrementedDMETPot, OrbitalEV);
            std::tuple< Eigen::MatrixXd, double, double > tmpTuple = std::make_tuple(DensityMatrix, Input.StartNorm, Input.StartLambda); // Add a new bias for the new solution. Starting N_x and lambda_x are here.
            Bias1.push_back(tmpTuple);
            SCFEnergy *= -1;
            SCFMDEnergyQueue1.push(std::pair<double, int>(SCFEnergy, ii));
            SCFMD1RDM1.push_back(DensityMatrix);
            SCFMDOccupied1.push_back(OccupiedOrbitals1);
            SCFMDVirtual1.push_back(VirtualOrbitals1);
            SCFMDCoeff1.push_back(CoeffMatrix);
            SCFMDOrbitalEV1.push_back(OrbitalEV);
        }
        for (int a = 0; a < NumSCFStates; a++) // Find RHF solutons after du step for all SCF states desired. We pick which ones to match later.
        {
            int NextIndex = SCFMDEnergyQueue.top().second;
...
{
    Eigen::VectorXd p;
    p = -1 * Gradient; // Hessian.colPivHouseholderQr().solve(-1 * Gradient);
...
void UpdatePotential(Eigen::MatrixXd &DMETPotential, InputObj &Input, Eigen::MatrixXd CoeffMatrix, Eigen::VectorXd OrbitalEV, std::vector< std::vector< int > > OccupiedByState, std::vector< std::vector< int > > VirtualByState, std::vector< Eigen::MatrixXd > FragmentDensities, std::vector< Eigen::MatrixXd> &FullDensities, std::ofstream &Output, std::vector< Eigen::MatrixXd > &FragmentRotations, std::vector< int > ImpurityStates, std::vector< int > BathStates)
{
    ...
    while(fabs(NormOfGrad) > 1 && fabs(dL) > 1E-1)
    {
        ...
        for (int ii = 0; ii < Input.NumSoln; ii++)
        {
            double SCFEnergy = SCF(Bias, ii + 1, DensityMatrix, Input, Output, Input.SOrtho, Input.HCore, AllEnergies, CoeffMatrix, OccupiedOrbitals, VirtualOrbitals, SCFCount, Input.MaxSCF, DMETPotential, OrbitalEV);
            std::tuple< Eigen::MatrixXd, double, double > tmpTuple = std::make_tuple(DensityMatrix, Input.StartNorm, Input.StartLambda); // Add a new bias for the new solution. Starting N_x and lambda_x are here.
            Bias.push_back(tmpTuple);
            SCFEnergy *= -1;
            SCFMDEnergyQueue.push(std::pair<double, int>(SCFEnergy, ii));
        ...
        {
            int NextIndex = SCFMDEnergyQueue.top().second;
            FullDensities[a] = 2 * SCFMD1RDM[NextIndex];
            SCFMDEnergyQueue.pop();
            // ************** Orbital lock method below ***********
